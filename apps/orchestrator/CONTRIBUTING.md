# Contributing to Flashwork

Thanks for being here. Flashwork is a small project with a real user base, and outside
...
[open an issue](https://github.com/abnerndr/flashwork-agents/issues/new?labels=documentation) and say so.
...
1. **Pick a [`good first issue`](https://github.com/abnerndr/flashwork-agents/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22).**
...
3. **Validate a platform.** Flashwork is Windows-first and still under-tested on Linux and macOS.
...
5. **Pick a [`help wanted`](https://github.com/abnerndr/flashwork-agents/issues?q=is%3Aissue+is%3Aopen+label%3A%22help+wanted%22) issue.**
...
```sh
git clone https://github.com/abnerndr/flashwork-agents.git
cd flashwork-agents
npm install
npm run app
```
...
`--status-working`, …). Flashwork ships 12 themes; a hardcoded `#10b981` looks right in dark mode and
...
- **Before/after screenshots or a GIF for any UI change.** Not optional — Flashwork is a visual app
...
Review is done by the maintainer ([@abnerndr](https://github.com/abnerndr)), who works on this alongside a
...
Flashwork spawns processes and creates PTYs from an unsigned binary, which trips Defender's ML
...
Flashwork is licensed under **AGPL-3.0-or-later**. By contributing, you agree that your contribution
is licensed under the same terms. The **Flashwork** name, logo, and branding are reserved for
