...
fn audit_record(
    ...
    let trailer = format!("Flashwork-Agent: {}", agent_id.unwrap_or("unknown"));
...
pub fn planning_audit_history(
    ...
    let format = format!(
        "%H{FIELD_SEP}%an{FIELD_SEP}%ct{FIELD_SEP}%s{FIELD_SEP}%(trailers:key=Flashwork-Agent,valueonly,separator=,){FIELD_SEP}%(trailers:key=Flashwork-Agent,valueonly,separator=,){RECORD_SEP}"
    );
...
mod tests {
    ...
    fn planning_repo() -> (std::path::PathBuf, String) {
        let root = std::env::temp_dir().join(format!("flashwork-planning-{}", nanoid!(8)));
    ...
    fn records_scoped_audit_commit_with_agent_trailer() {
        ...
        assert!(history[0].subject.contains("gsd(flashwork)"));
