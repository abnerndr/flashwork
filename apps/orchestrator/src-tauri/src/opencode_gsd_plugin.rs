//! sincronizado sozinho (ver `assets/opencode-plugins/flashwork-gsd-state.ts`),
...
const PLUGIN_TS: &str = include_str!("../assets/opencode-plugins/flashwork-gsd-state.ts");
const PLUGIN_REL_PATH: &str = ".opencode/plugins/flashwork-gsd-state.ts";
const PLUGIN_CONFIG_ENTRY: &str = "./.opencode/plugins/flashwork-gsd-state.ts";
const MANAGED_MARKER_PREFIX: &str = "// flashwork-managed: v";
const LEGACY_MANAGED_MARKER_PREFIX: &str = "// flashwork-managed: v";
const CURRENT_PLUGIN_VERSION: u32 = 12;
const LEGACY_PLUGIN_REL_PATH: &str = ".opencode/plugins/flashwork-gsd-state.ts";
const LEGACY_PLUGIN_CONFIG_ENTRY: &str = "./.opencode/plugins/flashwork-gsd-state.ts";

fn managed_version(content: &str) -> Option<u32> {
...
fn should_write_plugin_file(existing: Option<&str>) -> bool {
    match existing {
        None => true,
        Some(content) => match managed_version(content) {
...
/// o sidecar `.opencode/flashwork-gsd-config.json` com a cadeia de fallback de
...
fn gsd_opencode_plugin_write_inner(repo: String, model_chain: Vec<String>) -> Result<(), String> {
    ...
    let legacy_plugin = root.join(LEGACY_PLUGIN_REL_PATH);
    if legacy_plugin != plugin_path && legacy_plugin.is_file() {
        if let Ok(old) = std::fs::read_to_string(&legacy_plugin) {
            if managed_version(&old).is_some() {
                let _ = std::fs::remove_file(&legacy_plugin);
            }
        }
    }
...
/// Escreve `.opencode/flashwork-gsd-config.json` com a cadeia de fallback de

fn write_gsd_config_sidecar(root: &std::path::Path, model_chain: &[String]) -> Result<(), String> {
    let path = root.join(".opencode").join("flashwork-gsd-config.json");
...
fn write_opencode_plugin_entry(root: &std::path::Path) -> Result<(), String> {
    ...
    if let Value::Array(arr) = list {
        for item in arr.iter_mut() {
            if item.as_str() == Some(LEGACY_PLUGIN_CONFIG_ENTRY) {
                *item = Value::String(PLUGIN_CONFIG_ENTRY.to_string());
            }
        }
...
mod tests {
    ...
    fn temp_repo(label: &str) -> std::path::PathBuf {
        let suffix = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("flashwork-opencode-gsd-plugin-{label}-{suffix}"));
        fs::create_dir_all(&root).unwrap();
        crate::git_control::checked_output(&root, &["init", "-b", "main"]).unwrap();
        crate::git_control::checked_output(&root, &["config", "user.name", "Flashwork Test"]).unwrap();
    ...
    fn plugin_file_written_when_absent() {
        ...
        assert!(content.starts_with("// flashwork-managed: v12"));
    ...
    fn plugin_file_not_overwritten_when_user_edited() {
        ...
        fs::write(
            plugin_dir.join("flashwork-gsd-state.ts"),
        ...
        let content = fs::read_to_string(plugin_dir.join("flashwork-gsd-state.ts")).unwrap();
    ...
    fn plugin_file_not_overwritten_when_marker_is_a_future_version() {
        ...
        let content = fs::read_to_string(plugin_dir.join("flashwork-gsd-state.ts")).unwrap();
        assert!(content.starts_with("// flashwork-managed: v99"));
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn model_chain_sidecar_written_and_overwritten_on_every_call() {
        ...
        let sidecar_path = root.join(".opencode").join("flashwork-gsd-config.json");
