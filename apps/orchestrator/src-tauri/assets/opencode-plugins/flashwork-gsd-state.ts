// flashwork-managed: v12
// Gerado automaticamente pelo Flashwork. Seguro editar — se você mudar este
// arquivo, remova ou altere a linha acima ("flashwork-managed: vN") para
// impedir que o Flashwork sobrescreva suas mudanças em versões futuras.
//
...
// `.planning/.gsd-child-session` (o Flashwork usa esse arquivo pra abrir uma
...
// runtime de `.opencode/flashwork-gsd-config.json`, escrito pelo Flashwork a cada
...
// gratuito) antes de ser embutido no binário do Flashwork — ver
// docs/CHANGELOG.md.
...
const MODEL_CHAIN_CONFIG_PATH = '.opencode/flashwork-gsd-config.json'
 ...
 *  próprio Flashwork (`.planning/` — estado deste plugin — e `.opencode/`/
...
export const FlashworkGsdStatePlugin: Plugin = async ({ directory, worktree, client }) => {
  ...
  async function getOrCreateChildSession(mainSessionId: string): Promise<string | null> {
    ...
    try {
      ...
      const created: any = await (client as any).session.create({
        body: { directory: root, title: 'Flashwork · GSD Sync' },
      })
