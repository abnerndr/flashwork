...
/// formato real (escrito por `flashwork-gsd-state.ts` a partir de `todowrite`)
...
mod tests {
    ...
    fn temp_git_repo(label: &str) -> std::path::PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("flashwork-scheduler-{label}-{suffix}"));
    ...
    fn run_scheduler_tick_completes_running_task_when_worktree_planning_is_done() {
        ...
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let worktree = std::env::temp_dir().join(format!("flashwork-scheduler-wt-{suffix}"));
