...
pub const OPEN_PATH_EVENT: &str = "flashwork://open-path";
...
mod tests {
    ...
    fn ignores_argv0_and_reads_flag() {
        assert_eq!(
            extract_path_arg(&args(&["flashwork", "--open-path", "/tmp/x"])),
    ...
    fn reads_bare_positional() {
        assert_eq!(
            extract_path_arg(&args(&["flashwork", "/tmp/x"])),
            Some("/tmp/x".to_string())
        );
    }
    ...
    fn skips_unknown_flags_like_macos_psn() {
        assert_eq!(extract_path_arg(&args(&["flashwork", "-psn_0_1234"])), None);
        assert_eq!(
            extract_path_arg(&args(&["flashwork", "-psn_0_1234", "/tmp/x"])),
            Some("/tmp/x".to_string())
        );
    }
    ...
    fn no_args_means_no_target() {
        assert_eq!(extract_path_arg(&args(&["flashwork"])), None);
        assert_eq!(resolve_target_dir(&args(&["flashwork"]), Path::new("/")), None);
    }

    #[test]
    fn dot_resolves_against_caller_cwd() {
        let cwd = temp_root();
        let resolved = resolve_target_dir(&args(&["flashwork", "."]), &cwd).expect("resolved");
    ...
    fn relative_path_resolves_against_caller_cwd() {
        let base = temp_root();
        let nested = base.join("flashwork-cli-test-rel");
        std::fs::create_dir_all(&nested).expect("create dir");

        let resolved =
            resolve_target_dir(&args(&["flashwork", "flashwork-cli-test-rel"]), &base).expect("resolved");
    ...
    fn file_target_falls_back_to_its_directory() {
        let base = temp_root();
        let dir = base.join("flashwork-cli-test-file");
        std::fs::create_dir_all(&dir).expect("create dir");
        let file = dir.join("README.md");
        std::fs::write(&file, b"x").expect("write file");

        let resolved =
            resolve_target_dir(&args(&["flashwork", &file.to_string_lossy()]), Path::new("/"))
                .expect("resolved");
    ...
    fn missing_path_resolves_to_none() {
            let target = std::env::temp_dir().join("flashwork-cli-test-inexistente-xyz");
        let _ = std::fs::remove_dir_all(&target);
        assert_eq!(
            resolve_target_dir(
                &args(&["flashwork", &target.to_string_lossy()]),
                Path::new("/")
            ),
            None
        );
