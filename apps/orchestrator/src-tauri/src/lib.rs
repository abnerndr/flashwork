    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let is_wsl = std::env::var_os("WSL_DISTRO_NAME").is_some()
            || std::fs::read_to_string("/proc/version")
                ...
                .unwrap_or(false);
        if is_wsl {
            // Sem dispositivo Vulkan no WSL o Mesa cai no Zink e spamma
            // "ZINK: failed to choose pdev" / dri2. llvmpipe evita isso.
    ...
    builder
        .setup(move |app| {
            ...
            if let Some(window) = app.get_webview_window("main") {
                let _ = window.set_title("(DEV) Flashwork");
            }
            ...
            // `flashwork <path>` com o app fechado: guarda o alvo agora, o
            ...
            // reporta working/idle real de volta pro Flashwork (ver opencode_bridge.rs).
