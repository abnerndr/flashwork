#!/usr/bin/env bash
# Captura SÓ a janela do Flashwork (não a tela inteira), por CGWindowID.
#
# Resolve o problema que tínhamos: screencapture de tela cheia pegava o app
# errado / dependia de foco. Aqui achamos o window id do processo "flashwork" via
# CoreGraphics (Swift) e capturamos exatamente essa janela com `screencapture -l`.
#
# Uso: ./scripts/capture-window.sh [saida.png]   (default: /tmp/flashwork-window.png)
...
OUT="${1:-/tmp/flashwork-window.png}"

# Descobre o CGWindowID da maior janela on-screen cujo dono contém "flashwork".
...
// Filtra pelo DONO da janela (nome do processo) == flashwork — NÃO pelo título,
// que pode conter "flashwork" numa aba de navegador (ex.: o GitHub do projeto).
var best: (id: Int, area: Int)? = nil
for w in list {
    let owner = (w[kCGWindowOwnerName as String] as? String ?? "").lowercased()
    guard owner == "flashwork" || owner.hasPrefix("flashwork") else { continue }
...
if [[ -z "${WID:-}" ]]; then
  echo "capture-window: janela do Flashwork não encontrada (o app está aberto?)" >&2
  exit 1
fi
