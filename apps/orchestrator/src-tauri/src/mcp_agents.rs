...
/// `FLASHWORK_MCP_HOME` redirects every global config lookup to a scratch copy, so the
...
fn opencode_config_dir() -> Option<PathBuf> {
    if std::env::var_os("FLASHWORK_MCP_HOME").is_none() {
...
"#;
    ...
    fn codex_upsert_leaves_every_other_table_untouched() {
        let next = codex_upsert(CODEX_FIXTURE, &probe("flashwork-probe")).expect("writes");
    ...
    fn codex_upsert_round_trips_timeouts_and_passthrough() {
        let mut server = probe("flashwork-probe");
        ...
        let written = by_name(&parsed, "flashwork-probe");
    ...
    fn codex_upsert_keeps_a_hand_written_env_section_as_a_section() {
        let mut server = probe("flashwork-discord");
  ...
  "projects": { "D:\\repo": { "allowedTools": [] } }
    ...
    fn claude_upsert_keeps_top_level_key_order_and_unrelated_keys() {
        let next = ClaudeAdapter
            .upsert(CLAUDE_FIXTURE, &user_src(), &probe("flashwork-probe"))
            .expect("writes");
    ...
    fn codex_upsert_keeps_an_explicit_mcp_servers_header_and_its_comment() {
        let raw = "# my servers\n[mcp_servers]\n\n[mcp_servers.figma]\nurl = \"https://x\"\n";
        let next = codex_upsert(raw, &probe("flashwork-probe")).expect("writes");
