...
export function useXtermSession(params: {
  ...
  useEffect(() => {
    ...
    if (import.meta.env.DEV) {
      console.debug('[Flashwork][xterm] mount', {
        sessionPersistenceKey,
        retryKey,
    ...
    const requestWriteRecovery = (id: string, source: 'input' | 'paste', error: unknown) => {
      ...
      window.dispatchEvent(
        new CustomEvent('flashwork:terminal-restart-request', { detail: { ptyId: id } }),
      )
      window.setTimeout(() => {
    ...
    const runResize = () => {
      ...
      try {
        ...
        if (import.meta.env.DEV) console.error('[Flashwork][xterm] fit failed', error)
      ...
      try {
        ...
        if (import.meta.env.DEV) console.error('[Flashwork][xterm] refresh failed', error)
      }
      clampHorizontalScroll()
    ...
    window.addEventListener('flashwork:zoom-changed', onZoomChanged)
    window.addEventListener('flashwork:terminal-resize-request', onResizeRequest)
    ...
    async function start() {
      try {
        ...
        const handleResumeConflict = () => {
          ...
          onSessionIdRef.current?.(undefined)
          terminal.write(
            '\r\n\x1b[33m[Flashwork] Codex session is busy — opening a fresh session…\x1b[0m\r\n',
          )
        ...
        const exitUnlisten = await listenPtyExit(response.id, (payload) => {
          ...
          if (isAgent && elapsed < EARLY_EXIT_MS) {
            ...
            terminal.write(
              `\r\n\x1b[31m[Flashwork] ${command} encerrou imediatamente (code ${payload.code ?? '—'}).\x1b[0m\r\n` +
    ...
    return () => {
      if (import.meta.env.DEV) {
        console.debug('[Flashwork][xterm] unmount', {
          sessionPersistenceKey,
          retryKey,
          ptyId: ptyIdRef.current,
        })
      ...
      window.removeEventListener('flashwork:zoom-changed', onZoomChanged)
      window.removeEventListener('flashwork:terminal-resize-request', onResizeRequest)
      ro.disconnect()
      if (resizeTimer !== null) window.clearTimeout(resizeTimer)
