 ...
 *   package.json · src-tauri/tauri.conf.json · src-tauri/Cargo.toml · src-tauri/Cargo.lock (bloco flashwork)
 *
 * Uso:
...
// Cargo.lock: só o bloco do próprio crate (name = "flashwork").
const toAdd = [PKG, TAURI, CARGO]
if (existsSync(LOCK)) {
  bumpFile(LOCK, /name = "flashwork"\r?\nversion = "(\d+\.\d+\.\d+)"/, LOCK)
  toAdd.push(LOCK)
}
