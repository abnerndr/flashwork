...
fn check_token(request: &tiny_http::Request) -> bool {
    ...
    request
        .headers()
        .iter()
        .any(|h| h.field.as_str() == "X-Flashwork-Token" && h.value.as_str() == expected)
}
...
pub fn agent_hooks_settings_path() -> Result<String, String> {
    ...
    let path = std::env::temp_dir().join("flashwork-agent-hooks.json");
    ...
    let hook = serde_json::json!([
        { "hooks": [ {
            ...
            "timeout": 5,
            "headers": { "X-Flashwork-Token": token }
        } ] }
    ]);
...
pub fn start_listener(app: AppHandle) {
    std::thread::spawn(move || {
        ...
        for mut request in server.incoming_requests() {
            ...
            // O Flashwork emite `agent-spawn`; o front sobe um PTY worker. Campos:
            ...
            if url.starts_with("/codex") {
                ...
                let _ = request.respond(tiny_http::Response::from_string(
                    "queued no terminal codex do Flashwork",
                ));
                continue;
            }
