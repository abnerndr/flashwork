...
Notable user-facing changes to **Flashwork** are documented here. The format is based on
...
- **Preferences → Providers:** paste Anthropic, OpenAI, and Gemini API keys into the OS keyring
  ...
  environment variable for the coding-agent CLI (Anthropic → `ANTHROPIC_API_KEY` for Claude Code;
  OpenAI → `OPENAI_API_KEY` for Codex; Google → `GEMINI_API_KEY` / `GOOGLE_API_KEY` for Gemini CLI
  and Antigravity), with the key never traveling through the frontend and the toggle being on with
  nothing stored still spawning normally. **Refresh model list** pulls the latest catalog from each
  vendor's official HTTPS endpoint, merges it into the current snapshot, and keeps that snapshot
  untouched when a refresh fails. Replaces the OmniRoute key path removed in P09 — no OpenRouter or
  third-party gateway.
...
- New **MCP** tab in the right sidebar: a single place to see every MCP server configured on the
  ...
  unreachable, labelled with the date it was captured. Flashwork translates a server to each target's
...
- Agents that are not installed can now be installed from inside Flashwork. The onboarding agent step
  ...
  reporting success. Flashwork probes the machine for Node, npm, WinGet, Scoop and Chocolatey and only
...
- Freebuff and Mimo can now be installed from inside Flashwork like the other agents, with their
...
- The onboarding agent step was rebuilt as a table. Every agent is one row with its icon, the
  ...
  leaving the step, for when an agent was installed outside Flashwork.
...
- Invalid CLI overrides are rejected instead of being saved and launched. Existing invalid overrides
  are cleared automatically, preventing the Antigravity desktop application from opening when Flashwork
...
- Switching conversation from inside the CLI with `/new` or `/resume` now sticks. Flashwork pinned the
...
- Browser panes now offer app-first, balanced, and keep-alive resource modes. App-first is the default,
  and every mode releases hidden native webviews when Flashwork detects memory pressure.
...
- **Remote control is now off by default and stays off until you turn it on.** Flashwork used to open a
...
- GitHub repository cloning no longer depends on a hardcoded `D:\Projects` directory. The selected
  destination is now respected, with `~/Flashwork/<repository>` as the cross-platform fallback.
...
- Development builds on Linux now also apply the Flashwork icon at runtime. Packaged builds remain the
...
- The default profile image and generated app icons now use the dark Flashwork artwork.
...
Graphify became optional, the `flashwork` command gained direct project opening, and this release delivered
...
- Added the `flashwork` terminal command to open the current or selected directory in the existing app
...
- Secured the AgentCanvas local HTTP listener with a per-launch `X-Flashwork-Token` and limited request
  bodies to 1 MB.
...
- GSD test briefings are scoped to the files changed in the current session and exclude Flashwork-generated
...
- Repository-root discovery excludes GSD viewer panes and can resolve the shared Git root from any
  existing worktree.
- GSD viewer panes trust Flashwork-tracked child session IDs that OpenCode intentionally omits from normal
  session listings.
...
- Rebuilt the loading screen with animated Flashwork ASCII branding and dot-matrix progress.
...
[Unreleased]: https://github.com/abnerndr/flashwork-agents/compare/v1.5.0...HEAD
[1.5.0]: https://github.com/abnerndr/flashwork-agents/compare/v1.4.1...v1.5.0
[1.4.1]: https://github.com/abnerndr/flashwork-agents/compare/v1.4.0...v1.4.1
[1.4.0]: https://github.com/abnerndr/flashwork-agents/compare/v1.3.0...v1.4.0
[1.3.0]: https://github.com/abnerndr/flashwork-agents/releases/tag/v1.3.0
