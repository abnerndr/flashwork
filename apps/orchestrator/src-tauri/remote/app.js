...
const SESSION_KEY = 'flashwork.remote.session'
...
function renderHome(filter = '') {
  ...
  const sections = [...byGroup.entries()]
    ...
    .join('')
  app.innerHTML = `<div class="app-frame"><header class="topbar"><span class="brand-mark">›_</span><div class="brand-copy"><span class="eyebrow">Flashwork remote</span><h1>Workspace</h1></div>${connectionPill()}</header><main class="scroll"><section class="welcome"><div><span class="eyebrow">Remote workspace</span><h2>Choose an agent</h2><p>Continue a conversation from any group on your desktop.</p></div><div class="summary"><strong>${chatCount}</strong><span>active chats</span></div></section><label class="search"><span class="search-icon">⌕</span><input id="search" value="${escapeHtml(filter)}" placeholder="Search projects or agents" autocomplete="off" /></label><div id="workspace-list">${sections || '<div class="empty">No chats match this search.</div>'}</div></main></div>`
  document
...
function composerMarkup() {
  if (readOnly)
    return '<div class="notice">This device is paired in read-only mode. Sending messages is disabled in Flashwork.</div>'
...
function renderPairingRequired(message) {
  app.innerHTML = `<div class="fatal"><div class="fatal-card"><span class="brand-mark">›_</span><strong>Pairing link required</strong><p>${escapeHtml(message || 'Open Remote control in Flashwork and scan the QR code to connect this device.')}</p></div></div>`
}

function renderSessionLost(message) {
  dropSession()
  app.innerHTML = `<div class="fatal"><div class="fatal-card"><span class="brand-mark">!</span><strong>Remote session ended</strong><p>${escapeHtml(message || 'This device is no longer paired.')}</p><p>Open Remote control in Flashwork and scan a new QR code.</p></div></div>`
}

async function boot() {
  app.innerHTML =
    '<div class="loading"><div class="loading-card"><span class="brand-mark">›_</span><strong>Connecting to Flashwork</strong><p>Preparing your remote workspace…</p></div></div>'
  if (pairingToken) {
    try {
      await pair()
    } catch (error) {
