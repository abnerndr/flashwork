...
/** Casa o segmento `.flashwork/worktrees/` (ou legado `.flashwork/worktrees/`). */
const FLASHWORK_WORKTREES_SEGMENT = /[\\/]\.(?:flashwork|flashwork)[\\/]worktrees[\\/]/i

function deriveRepoRootFromWorktreeCwd(cwd: string): string {
  const match = cwd.match(FLASHWORK_WORKTREES_SEGMENT)
