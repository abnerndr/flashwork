...
// qualquer terminal opencode que o Flashwork spawnar, nao so os com Graphify
// habilitado. O plugin reporta session.idle/tool.execute.before de volta pro
// Flashwork via HTTP local, reaproveitando o listener ja existente em

use std::path::PathBuf;

const PLUGIN_FILE_NAME: &str = "flashwork-bridge.js";

const PLUGIN_SOURCE: &str = r#"                                                                                
// boot do app se o conteudo mudar).
//
                                                                                
// via o endpoint local passado em FLASHWORK_BRIDGE_ENDPOINT (injetado como env var
                                                                          
// quebrar a sessao do OpenCode se o Flashwork nao estiver rodando ou a porta tiver
// mudado.
export const FlashworkBridgePlugin = async ({ directory }) => {
  const endpoint = process.env.FLASHWORK_BRIDGE_ENDPOINT
  if (!endpoint) return {}
...
pub fn ensure_installed() {
    ...
    let _ = std::fs::remove_file(dir.join("flashwork-bridge.js"));
...
mod tests {
    ...
    fn plugin_source_exports_the_expected_hooks() {
        assert!(PLUGIN_SOURCE.contains("export const FlashworkBridgePlugin"));
        assert!(PLUGIN_SOURCE.contains("session.idle"));
        assert!(PLUGIN_SOURCE.contains("tool.execute.before"));
        assert!(PLUGIN_SOURCE.contains("FLASHWORK_BRIDGE_ENDPOINT"));
