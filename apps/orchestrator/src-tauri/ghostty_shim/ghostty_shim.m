...
@class FlashworkGhosttyView;

#define FLASHWORK_MAX_SURFACES 256
static ghostty_surface_t g_surfaces[FLASHWORK_MAX_SURFACES];
static FlashworkGhosttyView *g_views[FLASHWORK_MAX_SURFACES];
...
static void flashwork_draw_surface(ghostty_surface_t s) {
    if (!s) return;
    ghostty_surface_draw(s);
    g_draw_count++;
}

static void flashwork_display_link_start(void);
static void flashwork_display_link_stop(void);

static void flashwork_register_surface(ghostty_surface_t s, FlashworkGhosttyView *v) {
    if (g_surface_count < FLASHWORK_MAX_SURFACES) {
        g_surfaces[g_surface_count] = s;
        g_views[g_surface_count] = v;
        g_surface_count++;
        // Primeira surface viva → liga o render contínuo.
        if (g_surface_count == 1) flashwork_display_link_start();
    }
}
static void flashwork_unregister_surface(ghostty_surface_t s) {
    for (int i = 0; i < g_surface_count; i++) {
        if (g_surfaces[i] == s) {
            ...
            if (g_surface_count == 0) flashwork_display_link_stop();
            return;
        }
    }
}
static void flashwork_draw_all(void) {
    for (int i = 0; i < g_surface_count; i++) {
        if (g_surfaces[i]) flashwork_draw_surface(g_surfaces[i]);
    }
}
...
@interface FlashworkRenderTicker : NSObject
- (void)tick:(NSTimer *)t;
@end
@implementation FlashworkRenderTicker
- (void)tick:(NSTimer *)t {
    (void)t;
...
static FlashworkRenderTicker *g_render_ticker = NULL;

static void flashwork_display_link_start(void) {
    if (g_render_timer != NULL) return;
    if (g_render_ticker == NULL) g_render_ticker = [[FlashworkRenderTicker alloc] init];
...
// O userdata da surface é a FlashworkGhosttyView dona dela (setado em cfg.userdata
...
static ghostty_surface_t flashwork_surface_for_userdata(void *userdata);
...
static bool shim_read_clipboard_cb(void *userdata,
    ...
    ghostty_surface_t surface = flashwork_surface_for_userdata(userdata);
...
static ghostty_input_mods_e flashwork_mods(NSEventModifierFlags f) {
...
@interface FlashworkGhosttyView : NSView <NSTextInputClient>
...
@implementation FlashworkGhosttyView
...
- (NSString *)flashwork_composeForEvent:(NSEvent *)event deadPending:(BOOL *)outDead {
    *outDead = NO;
...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
    ...
    key.mods = flashwork_mods(event.modifierFlags);
    ...
    key.consumed_mods = flashwork_mods(consumed);
...
- (void)keyDown:(NSEvent *)event {
    ...
    if (ctrlOrCmd) {
        ...
        BOOL dead = NO;
        NSString *composed = [self flashwork_composeForEvent:event deadPending:&dead];
        if (composed == nil) {
...
- (void)mouseDown:(NSEvent *)event {
    ...
    ghostty_surface_mouse_pos(self.surface, p.x, p.y, flashwork_mods(event.modifierFlags));
    ghostty_surface_mouse_button(self.surface, GHOSTTY_MOUSE_PRESS,
                                 GHOSTTY_MOUSE_LEFT, flashwork_mods(event.modifierFlags));
}
...
bool flashwork_ghostty_ensure_app(void) {
...
static void flashwork_install_cmd_monitor(void) {
    ...
    g_cmd_key_monitor = [NSEvent
        ...
        if (![fr isKindOfClass:[FlashworkGhosttyView class]]) return event;
        FlashworkGhosttyView *v = (FlashworkGhosttyView *)fr;
...
flashwork_surface_t flashwork_ghostty_surface_new(void *superview,
                                            const char *cwd,
                                            const char *command,
                                            double scale_factor) {
    if (!flashwork_ghostty_ensure_app()) return NULL;
    ...
    FlashworkGhosttyView *view = [[FlashworkGhosttyView alloc] initWithFrame:parent.bounds];
    view.wantsLayer = YES;
    [parent addSubview:view];
    flashwork_install_cmd_monitor();
    ...
    flashwork_register_surface(surface, view);
    ...
    return (flashwork_surface_t)surface;
}

// Resolve o userdata da surface (a FlashworkGhosttyView dona) de volta pra
// surface. Usado pelos callbacks de clipboard — ver forward declaration.
static ghostty_surface_t flashwork_surface_for_userdata(void *userdata) {
    if (userdata == NULL) return NULL;
    FlashworkGhosttyView *v = (__bridge FlashworkGhosttyView *)userdata;
    return v.surface;
}

// Recupera a FlashworkGhosttyView associada a uma surface (busca no registro).
static FlashworkGhosttyView *flashwork_view_for_surface(ghostty_surface_t s) {
    for (int i = 0; i < g_surface_count; i++) {
...
void flashwork_ghostty_surface_set_frame(flashwork_surface_t surface,
                                      double x, double y,
                                      double w, double h) {
    if (surface == NULL) return;
    FlashworkGhosttyView *v = flashwork_view_for_surface((ghostty_surface_t)surface);
    if (v) v.frame = NSMakeRect(x, y, w < 1 ? 1 : w, h < 1 ? 1 : h);
}

void flashwork_ghostty_surface_set_hidden(flashwork_surface_t surface, bool hidden) {
    if (surface == NULL) return;
    FlashworkGhosttyView *v = flashwork_view_for_surface((ghostty_surface_t)surface);
    if (v) v.hidden = hidden;
}

void flashwork_ghostty_surface_set_size(flashwork_surface_t surface,
...
void flashwork_ghostty_surface_set_content_scale(flashwork_surface_t surface,
                                              double x, double y) {
    if (surface == NULL) return;
    ghostty_surface_set_content_scale((ghostty_surface_t)surface, x, y);
}

void flashwork_ghostty_surface_set_focus(flashwork_surface_t surface, bool focused) {
    if (surface == NULL) return;
    ghostty_surface_set_focus((ghostty_surface_t)surface, focused);
}

void flashwork_ghostty_surface_draw(flashwork_surface_t surface) {
    if (surface == NULL) return;
    flashwork_draw_surface((ghostty_surface_t)surface);
}
...
bool flashwork_ghostty_surface_process_exited(flashwork_surface_t surface) {
    if (surface == NULL) return false;
    return ghostty_surface_process_exited((ghostty_surface_t)surface);
}

unsigned long long flashwork_ghostty_draw_count(void) {
    return g_draw_count;
}

bool flashwork_ghostty_test_type_key(flashwork_surface_t surface,
...
const char *flashwork_ghostty_test_last_key_text(void) {
    return g_last_key_text;
}
bool flashwork_ghostty_test_last_key_composing(void) {
    return g_last_key_composing;
}

bool flashwork_ghostty_test_ime_compose(flashwork_surface_t surface,
                                     const char *marked,
                                     const char *final) {
    if (surface == NULL) return false;
    FlashworkGhosttyView *v = flashwork_view_for_surface((ghostty_surface_t)surface);
...
void flashwork_ghostty_surface_free(flashwork_surface_t surface) {
    if (surface == NULL) return;
    FlashworkGhosttyView *v = flashwork_view_for_surface((ghostty_surface_t)surface);
    flashwork_unregister_surface((ghostty_surface_t)surface);
...
void flashwork_ghostty_kill_all(void) {
    while (g_surface_count > 0) {
        ghostty_surface_t s = g_surfaces[0];
        FlashworkGhosttyView *v = g_views[0];
        flashwork_unregister_surface(s);
        if (s) ghostty_surface_free(s);
        [v removeFromSuperview];
    }
    fprintf(stderr, "[flashwork-diag] kill_all -> count=%d\n", g_surface_count);
}

void flashwork_ghostty_app_tick(void) {
    if (g_app) ghostty_app_tick(g_app);
}

void flashwork_ghostty_surface_send_text(flashwork_surface_t surface,
                                      const char *utf8,
                                      size_t len) {
    ...
    size_t flashwork_ghostty_surface_read_screen(flashwork_surface_t surface,
                                          char *out,
                                          size_t cap) {
