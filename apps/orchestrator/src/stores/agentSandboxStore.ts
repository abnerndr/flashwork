...
const SPAWN_BRIDGE_PROMPT = `You are the Planner Claude inside Flashwork. You are the parent orchestrator. When the user asks you to delegate work, create a real Codex worker through Flashwork's local bridge instead of doing the work yourself. Use a short self-contained task and include parent_id='lead'. Example: $body=@{agent='codex';task='Create worker-proof.txt containing READY';cwd=(pwd).Path;parent_id='lead'}|ConvertTo-Json -Compress; Invoke-RestMethod "$env:FLASHWORK_AGENT_HOOKS_ENDPOINT/spawn" -Method Post -Headers @{'X-Flashwork-Token'=$env:FLASHWORK_AGENT_HOOKS_TOKEN} -ContentType 'application/json' -Body $body. Keep task under 300 characters. Flashwork injects the worker's completed response back into your terminal, so review it and send follow-up work through the same bridge or composer. Never claim a worker completed work until its response arrives. If a worker exists, do not perform its delegated task yourself or edit its files.`
  ...
  const detail =
    typeof detailSource === 'string'
      ...
      : detailSource && typeof detailSource === 'object'
        ...
        : ''
  return `[Flashwork hook] ${eventName}${tool}${detail ? `: ${detail.replace(/[\r\n]+/g, ' ').slice(0, 420)}` : ''}`
}
...
function agentInput(command: SandboxNode['command'], from: string, text: string): string {
  const message = `[Flashwork message from ${from}] ${text.trim()}`
            ...
            const cleanup = await listenCodexAppServer(appServerId, (event) => {
              ...
              if (method === 'turn/completed') {
                ...
                if (reply && parent?.ptyId) {
                  const relay = `[Flashwork reply from ${node.label}] ${reply}`
                  ...
                  nodes: state.nodes.map((item) =>
                    item.id === node.id
                      ? {
                          ...item,
                          status: 'idle',
                          output: `${item.output ?? ''}\n\n[Flashwork] Turn completed. Ready for another message.\n`,
                        }
                      : item,
                  ),
                }))
                  ...
                  nodes: state.nodes.map((item) =>
                    item.id === node.id
                      ? {
                          ...
                          output: `${item.output ?? ''}\n\n[Flashwork] Codex connection closed.\n`,
                        }
                      : item,
                  ),
                }))
