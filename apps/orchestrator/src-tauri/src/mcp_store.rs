  ...
  "projects": {
    ...
    fn claude_local_write_lands_in_the_matching_project_entry() {
        ...
        apply_to_source(
            McpAgent::Claude,
            &source,
            Mutation::Upsert(Box::new(probe_server("flashwork-probe"))),
            None,
        )
        .expect("writes");
        ...
        assert!(written.contains("flashwork-probe"));
    ...
    fn writing_backs_up_and_leaves_the_rest_of_the_file_alone() {
        ...
        assert_eq!(report.changed, vec!["flashwork-probe".to_string()]);
    ...
    fn a_field_the_agent_cannot_express_blocks_the_write() {
        ...
        let mut server = probe_server("flashwork-probe");
        server
            .env
