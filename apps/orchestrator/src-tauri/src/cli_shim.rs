...
//! | Windows         | `%LOCALAPPDATA%\Flashwork\bin\flashwork.cmd` | registrado em `HKCU\Environment` |
//!
...
#[cfg(windows)]
const SHIM_FILE_NAME: &str = "flashwork.cmd";
#[cfg(not(windows))]
const SHIM_FILE_NAME: &str = "flashwork";
...
fn shim_bin_dir() -> Result<PathBuf, String> {
    #[cfg(windows)]
    {
        ...
        Ok(base.join("Flashwork").join("bin"))
    }
    #[cfg(not(windows))]
    {
        let home =
...
fn unix_shim_script(target_marker: &str, launch: &str) -> String {
    format!(
        r#"#!/bin/sh
# flashwork — abre um diretório no Flashwork a partir do terminal.
#
# Gerado automaticamente pelo Flashwork (Configurações ▸ Integrações ▸ Comando de
# terminal). Não edite à mão: reinstale por lá, principalmente depois de mover
# ou reinstalar o app.
#
# flashwork            → abre o diretório atual
# flashwork .          → idem
# flashwork ~/projeto  → abre o diretório informado
#
# FLASHWORK_TARGET_BIN: {target_marker}
...
if [ ! -d "$target" ]; then
  echo "flashwork: diretório não encontrado: $target" >&2
  exit 1
fi
...
fn render_shim(binary: &Path) -> Result<String, String> {
    ...
    #[cfg(windows)]
    {
        ...
        return Ok(format!(
            r#"@echo off
rem flashwork - abre um diretorio no Flashwork a partir do terminal.
rem
rem Gerado automaticamente pelo Flashwork (Configuracoes > Integracoes > Comando de
rem terminal). Nao edite a mao: reinstale por la, principalmente depois de mover
rem ou reinstalar o app.
rem
rem FLASHWORK_TARGET_BIN: {binary}
...
if not exist "%target%\" (
  echo flashwork: diretorio nao encontrado: %target% 1>&2
  exit /b 1
)
...
fn shim_targets_binary(contents: &str, binary: &Path) -> bool {
    let Some(line) = contents
        .lines()
        .find_map(|line| line.split_once("FLASHWORK_TARGET_BIN:"))
...
mod tests {
    ...
    fn shim_mentions_target_binary() {
        let binary = PathBuf::from("/opt/flashwork/flashwork");
    ...
    fn stale_shim_is_detected() {
        let old = PathBuf::from("/opt/flashwork-antigo/flashwork");
        let script = render_shim(&old).expect("script");
        assert!(!shim_targets_binary(
            &script,
            Path::new("/opt/flashwork-novo/flashwork")
        ));
    }
    ...
    fn shim_without_marker_counts_as_stale() {
        assert!(!shim_targets_binary(
            "#!/bin/sh\necho oi\n",
            Path::new("/opt/flashwork/flashwork")
        ));
    }
    ...
    fn shim_defaults_to_current_dir_and_is_a_posix_script() {
        let script = render_shim(Path::new("/opt/flashwork/flashwork")).expect("script");
    ...
    fn generated_shim_is_valid_shell_syntax() {
        ...
        for binary in [
            "/opt/flashwork/flashwork",
            "/Applications/Flashwork.app/Contents/MacOS/Flashwork",
        ] {
            let script = render_shim(Path::new(binary)).expect("script");
            let file = std::env::temp_dir().join(format!(
                "flashwork-shim-syntax-{}.sh",
                binary.replace(['/', '.'], "_")
            ));
    ...
    fn single_quotes_in_path_are_escaped() {
        let script = render_shim(Path::new("/opt/flashwork's app/flashwork")).expect("script");
