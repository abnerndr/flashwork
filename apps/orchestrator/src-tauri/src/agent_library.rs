...
const MARKER: &str = "gerado pelo Flashwork";
const LEGACY_MARKER: &str = "gerado pelo Flashwork";

#[derive(Serialize)]
pub struct InstalledAgent {
    pub name: String,
    /// Whether the file contains the Flashwork (or legacy Flashwork) marker and can be removed safely.
    pub from_flashwork: bool,
}
...
fn list_installed_agents_inner(folder: String) -> Vec<InstalledAgent> {
    ...
    let mut agents: Vec<InstalledAgent> = entries
        ...
        .filter_map(|entry| {
            ...
            let from_flashwork = fs::read_to_string(&path)
                .map(|c| c.contains(MARKER) || c.contains(LEGACY_MARKER))
                .unwrap_or(false);
            Some(InstalledAgent { name, from_flashwork })
        })
        .collect();
