...
//!
//! Estrutura de `.planning/` (ver `assets/opencode-plugins/flashwork-gsd-state.ts`):
...
/// dedicada (`gsd_record_step`, em `flashwork-gsd-state.ts`) — nunca por parsing
...
pub struct PlanningStatus {
    ...
    /// (`flashwork-gsd-state.ts`) com o plano passo a passo, incluindo o
...
mod tests {
    ...
    fn temp_dir(label: &str) -> std::path::PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("flashwork-planning-gate-{label}-{suffix}"));
