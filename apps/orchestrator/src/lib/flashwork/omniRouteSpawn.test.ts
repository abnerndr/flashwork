...
import { isOmniRouteAgent, resolveOmniRouteSpawnEnv } from './omniRouteSpawn'
...
describe('isOmniRouteAgent', () => {
  it('skips shell and empty targets', () => {
    expect(isOmniRouteAgent(null)).toBe(false)
    expect(isOmniRouteAgent(undefined)).toBe(false)
    expect(isOmniRouteAgent('shell')).toBe(false)
  })

  it('accepts agent CLIs', () => {
    expect(isOmniRouteAgent('claude')).toBe(true)
    expect(isOmniRouteAgent('codex')).toBe(true)
    expect(isOmniRouteAgent('opencode')).toBe(true)
    expect(isOmniRouteAgent('gemini')).toBe(true)
    expect(isOmniRouteAgent('copilot')).toBe(true)
  })
})
...
describe('resolveOmniRouteSpawnEnv', () => {
  ...
  it('skips gateway env for shell PTYs', async () => {
  ...
  it('skips gateway env when the spawn has no agent', async () => {
    const env = await resolveOmniRouteSpawnEnv({}, { omniRouteEnabled: true }, null)
    expect(env).toEqual({})
    expect(health).not.toHaveBeenCalled()
  })
  ...
  it('skips gateway env when enabled but key is missing', async () => {
    ...
    const env = await resolveOmniRouteSpawnEnv({}, { omniRouteEnabled: true }, 'claude')
    expect(env).toEqual({})
  ...
  it('merges gateway env when opted in, healthy, and keyed', async () => {
    health.mockResolvedValue(okHealth())
    getKey.mockResolvedValue('gw-key')
    const env = await resolveOmniRouteSpawnEnv({ FOO: '1' }, { omniRouteEnabled: true }, 'claude')
  ...
  it('does not throw when health lookup fails', async () => {
    ...
    getKey.mockResolvedValue('gw-key')
    await expect(
      resolveOmniRouteSpawnEnv({}, { omniRouteEnabled: true }, 'claude'),
    ).resolves.toEqual({})
  })
})
