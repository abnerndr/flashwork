...
//!   `<repo>/.flashwork/worktrees/<id>/`, compartilhando o `.git` do repo. Nesse
...
fn worktrees_base(root: &Path) -> PathBuf {
    root.join(".flashwork").join("worktrees")
}
...
pub(crate) fn worktree_provision_inner(
    ...
    let branch = format!("flashwork/floor-{id}");
...
pub(crate) fn worktree_remove_inner(
    ...
    // que o destino esteja dentro de `<repo>/.flashwork/worktrees`.
    let canon_base = base
        .canonicalize()
...
mod tests {
    ...
    fn temp_repo() -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("flashwork-worktrees-{suffix}"));
    ...
    fn fetch_branch_brings_local_copy_work_into_main_repo() {
        ...
        checked_output(env, &["config", "user.name", "Flashwork Test"]).unwrap();
        checked_output(env, &["config", "user.email", "flashwork@example.invalid"]).unwrap();
        checked_output(env, &["commit", "-am", "copy work"]).unwrap();

        let missing = git_command(
            &root,
            &["rev-parse", "--verify", "refs/heads/flashwork/floor-fetchme"],
        )
        .unwrap();
        assert!(
