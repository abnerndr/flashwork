...
fn git_init_inner(path: String) -> Result<String, String> {
    ...
    let identity = [
        ...
        "user.name=Flashwork",
        "-c",
        "user.email=flashwork@localhost",
    ];
    let mut commit_args: Vec<&str> = identity.to_vec();
    commit_args.extend(["commit", "-m", "Commit inicial (Flashwork)"]);
...
/// `.opencode/flashwork-gsd-config.json`, `.planning/goal.md` etc. como se

/// Flashwork escrevendo essa infraestrutura na worktree.
fn is_flashwork_infra_path(path: &str) -> bool {
...
fn git_diff_summary_inner(
    ...
    entries.retain(|entry| !is_flashwork_infra_path(&entry.path));
...
mod tests {
    ...
    fn main_repository_root_resolves_same_root_from_a_linked_worktree() {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("flashwork-main-repo-root-{suffix}"));
    ...
    fn performs_stage_unstage_discard_and_commit() {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("flashwork-git-control-{suffix}"));
        fs::create_dir_all(&root).unwrap();
        let root_string = root.to_string_lossy().into_owned();
        let run = |args: &[&str]| checked_output(&root, args).unwrap();
        run(&["init"]);
        run(&["config", "user.name", "Flashwork Test"]);
        run(&["config", "user.email", "flashwork@example.invalid"]);
    ...
    fn git_diff_summary_includes_uncommitted_worktree_changes() {
        ...
        assert!(
            ...
            "infraestrutura do Flashwork (.planning/, .opencode/, opencode.json) nunca deve aparecer: {with_worktree:?}"
        );
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("flashwork-git-control-{label}-{suffix}"));
        fs::create_dir_all(&root).unwrap();
        root
    }
